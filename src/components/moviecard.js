...
function Moviecard({title,poster,language,voteAverage,voteCount,size}) {
  ...
  return (
    <TouchableOpacity activeOpacity={0.9} style={{ borderRadius: 20 }} resizeMode="cover">
      <ImageBackground imageStyle={{borderRadius:20}} style={styles.container} source={{uri:getPoster(poster)}}>
       <View style={styles.imdbIconContainer}>
        <Image source={images.IMDB} style={styles.imdbImage}/>
        <Text style={{color:"white",fontWeight:'700'}}>{voteAverage}</Text>
       </View>
       {
        liked?
           <Ionicons name="heart-outline" size={24} color="white" style={{position:"absolute",bottom:10,left:10}} onPress={()=>setLiked(!liked)}/>
     :  <Ionicons name="heart" size={24} color="red" style={{position:"absolute",opacity:0.7,bottom:10,left:10}} onPress={()=>setLiked(!liked)}/>
       }
      </ImageBackground>
      <View>
        <Text numberOfLines={3} style={{ width: 230 }}>{title}</Text>
        <View style={styles.movieSubtitleContainer}>
          <Text style={styles.movieSubtitle}>{getLanguage(language).english_name}</Text>
          <View style={styles.rowAndcenter}>
            <Ionicons name="ios-heart" size={17} style={{ marginRight: 10 }} color="red" />
            <Text>{voteCount}</Text>
          </View>
        </View>
      </View>
